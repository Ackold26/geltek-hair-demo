...
(function () {
  ...
  /* ---------- Единый обработчик прокрутки: два типа эффектов на элементах секций ----------
     ...
     - data-par="<амплитуда в cqw>" — относительное смещение translateY от -амплитуда/2 до
       +амплитуда/2 (декоративный «лёгкий эффект движения» экранов 2/3/4/5/6/7/10). Амплитуда в
       разметке — исходное значение из длины стрелки макета (px/19.2), без потолка: потолок в
       3cqw гасил бы амплитуды 24.531/27.031/12.969 почти до нуля, и эффект стал бы «почти
       незаметен» везде, кроме первого экрана (тот идёт по отдельному механизму
       data-scroll-y0/y1). Смещение равно длине стрелки из макета без урезания;
