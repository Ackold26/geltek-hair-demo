...
(function () {
  ...
  /* ---------- Единый обработчик прокрутки: два типа эффектов на элементах секций ----------
     ...
     - data-par="<амплитуда в cqw>" — относительное смещение translateY от -амплитуда/2 до
       ...
       в разметке — исходное значение из длины стрелки макета (px/19.2), без потолка (H, 28.07):
       потолок 3cqw из карты REPAIR_MAP_2707_v2 гасил разметочные амплитуды 24.531/27.031/12.969
       почти до нуля — эффект стал «почти незаметен» везде, кроме первого экрана (тот идёт по
       отдельному, некапнутому механизму data-scroll-y0/y1). По новой карте (REPAIR_MAP_2807, H)
       смещение должно равняться длине стрелки из макета без урезания, поэтому PAR_MAX убран;
